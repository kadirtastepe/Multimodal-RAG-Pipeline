...
A working prototype has been developed using Flask API's and deployed to Cloud Foundry. Integrates SpaCy-based anonymizer for anonymized data processing, a PostgreSQL vector database for similarity search, and an Azure OpenAI-powered conversational interface with LangGraph orchestration for context-aware responses. The method's versatility accross diverse document types, large languange models and allowing rapid database change hold early promise for robust performance for future applications.

## Architecture:
This project implements a three-stage microservices architecture deployed on SAP's Cloud Foundry platform to transform raw enterprise documentation into an intelligent, anonymized knowledge base. The Anonymizer Service continuously fetches documentation from GitHub repositories and Jira, using SpaCy's transformer-based NLP to identify and redact sensitive information (names, IDs, emails, phone numbers, credentials) while preserving semantic structure, then uploads the anonymized content to AWS S3-compatible object storage. The Vectorizer Service processes this anonymized data by generating semantic embeddings using OpenAI's text-embedding-3-large model through batch processing with memory monitoring, storing the vectors in a PostgreSQL database with pgvector extension for efficient similarity search. Finally, the API Service provides the conversational interface through LangGraph orchestration, combining semantic search with GPT-5 generation to deliver contextual, anonymized responses via Slack integration while maintaining thread-based conversation state and memory for seamless multi-turn interactions. All microservices are integrated to OpenSearch cluster for monitoring. 
